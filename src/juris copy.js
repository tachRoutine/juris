...
class DOMRenderer {
    constructor(juris) {
        ...
        this.XLINK_ATTRS = new Set(['href', 'xlink:href']);
        this.BOOL_ATTRS = new Set(['checked', 'selected', 'disabled', 'hidden', 'readonly', 'multiple']);
    ...
    applyProp(element, key, value, context) {
        switch (key) {
            ...
            case 'text':
                return this.setValue(value, v => element.textContent = v, context, element);
            case 'style':
                return this.setStyle(element, value, context);
            case 'innerHTML':
                return this.setValue(value, v => this.safeSetInnerHTML(element, v), context, element);
            case 'className':
            case 'class':
                return this.setValue(value, v => element.className = v, context, element);
            case 'key':
    ...
    // ===== STYLE HANDLING WITH CSS EXTRACTION =====
    ...
    setValue(value, setter, context, element = null) {
        if (typeof value === 'function') {
            this.makeReactive(value, setter, context, element);
        } else if (value?.then) {
            this.handleAsync(value, setter, element);
        } else {
            setter(value);
        }
    }
    ...
    makeReactive(fn, setter, context, element = null) {
        // Prevent multiple subscriptions by checking if already reactive
        if (element && element._jurisReactiveSetup) {
            return; // Already setup, don't create duplicate
        }
        ...
        if (element) {
            element._jurisReactiveSetup = true;
        }
    ...
    // Async handling
    handleAsync(promise, setter, element = null) {
        promisify(promise)
    ...
    setAttributeSmart(element, attr, value) {
        ...
        if (this.BOOL_ATTRS.has(attr)) {
            ...
            // Set property for form elements
            if (attr === 'checked') element.checked = bool;
            if (attr === 'disabled') element.disabled = bool;
            if (attr === 'selected') element.selected = bool;
            return;
        }
        ...
        const update = () => {
            ...
            try {
                ...
                if (result?.then) {
                    promisify(result)
                        .then(resolved => {
                            if (!placeholder._jurisActive || !placeholder.parentNode) return;
                            const newNode = this.render(resolved) || document.createTextNode('');
                            newNode._jurisActive = true;
                        ...
                        .catch(error => {
                            if (placeholder._jurisActive && placeholder.parentNode) {
    ...
    setupIndicators(elementId, config) {
        // Placeholder setup for loading indicators
    ...
    clearCaches() {
        ...
        // Clear CSS extractor caches if available
    ...
    getMemoryStats() {
        ...
        // Add CSS extractor stats if available
    ...
    _createPlaceholder(text) { return document.createTextNode(text); }
    _setStaticAttribute(el, attr, val) { this.setAttributeSmart(el, attr, val); }
    _handleText(el, text, subs) { this.setValue(text, v => el.textContent = v, {subs, events: []}, el); }
    _handleStyle(el, style, subs) { this.setStyle(el, style, {subs, events: []}); }
    ...
    _getPlaceholderConfig() { return {text: 'Loading...', className: 'loading'}; }
}
