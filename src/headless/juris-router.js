	...
	const Router = (props = {}, context) => {
		...
		const defaultConfig = {
			...
			disregardParams: ['__state', '__v'], // To disregard specific URL parameters for future features
			// Debug options
		...
		function parseQueryString(queryString) {
			...
			for (const pair of pairs) {
				...
				if (key) {
					...
					if (config.disregardParams.includes(decodedKey)) {
            continue;
        	}
	...
	if (typeof window !== 'undefined') {
		...
		Object.freeze(window.Router);
		Object.freeze(window.Router.prototype);
	}
